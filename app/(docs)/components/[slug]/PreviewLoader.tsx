...
import {
  ...
  Skeleton, Switch,
  Tooltip, Slider, Avatar, EmptyState, Breadcrumb,
...
function ToggleGroupPreview() {
  ...
  const [underlineVal, setUnderlineVal] = useState('Overview')
  return (
    <div className="flex flex-col gap-4">
      ...
      <ToggleGroup variant="underline" options={['Overview', 'Details', 'Settings']} value={underlineVal} onChange={(v) => setUnderlineVal(v as string)} />
    </div>
  )
}
  ...
  return (
    <div className="flex gap-6 items-center">
      <NotificationBadge count={5}>
        <div className="w-8 h-8 rounded-[6px] bg-[var(--n200)] flex items-center justify-center text-[12px]">N</div>
      </NotificationBadge>
      <NotificationBadge dot>
        <div className="w-8 h-8 rounded-[6px] bg-[var(--n200)] flex items-center justify-center text-[12px]">M</div>
      </NotificationBadge>
    </div>
  )
}
...
export function PreviewLoader({ slug }: { slug: string }) {
